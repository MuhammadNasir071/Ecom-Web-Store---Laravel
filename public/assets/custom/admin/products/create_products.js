api['storeProducts'] = ajax_path + '/admin/products';
api['getShippingType'] = ajax_path + '/admin/shipping/types/:id';

var add_product_submit_btn = $('#add_product_btn');
...
myDropzone = new Dropzone('div#imageUpload', {
    ...
    init: function () {
        ...
        $('.add-button').on("click", function (e) {
            ...
            button_status(add_product_submit_btn, true, 'Creating');

            if ($('#categories-add-div').find('.selected_category_tag').length < 1) {
                toastr.error('Please select a category for your product');
                button_status(add_product_submit_btn, false);
                return;
            }

            if (myDropzone.getAcceptedFiles().length === 0) {
                $("#image-error").text("Upload at least 1 image");
                button_status(add_product_submit_btn, false);
                return false;
            }
            tinyMCE.triggerSave();
    ...
    completemultiple: function (file) {
        console.log(file, "completemultiple");
        setTimeout(function () {
            button_status(add_product_submit_btn, false);
        }, 1000);
    },
...
function getFormData() {
    ...
    data.append('product_description', $('#description').val());
    ...
    var product_managed_by = $('#managed_by_vendor').val();
    ...
    // check product stock owner. if vendor the get name.
    ...
    // product managed by.
    if ($('#managed_by_vendor').is(":checked")) {
        product_managed_by = $('#managed_by_vendor').val();
    }
    else {
        product_managed_by = $('#managed_by_giftpoke').val();
    }

    // Check promotion continue or limited
    ...
    data.append('product_managed_by', product_managed_by);
