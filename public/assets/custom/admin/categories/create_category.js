...
// categories creeting/updating form btn  
var add_categories_btn = $('.add_categories_btn');
var update_categories_btn = $('.update_categories_btn');
...
$('body').on('submit', '.add_category', function (e) {
    ...
    button_status(add_categories_btn, true, "loading");
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.responseCode == 200) {
                ...
                setTimeout(function () {
                    button_status(add_categories_btn, false);
...
$('body').on('submit', '.update_category', function (e) {
    ...
    data.append('parent_id', parent_id);
    button_status(update_categories_btn, true, "Updating");
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.responseCode == 200) {
                toastr.success(response.message);
                setTimeout(function () {
                    button_status(update_categories_btn, false);
                    window.location.reload();
                }, 1500);
            }
            else {
...
function createSelectedCategoryTag(value, text, index) {
    let div = '<div class="mx-1 selected_category_tag d-flex justify-content-between" data-id="' + value + '"  data-index="' + index + '" data-text="' + text + '"  style="max-width: 150px; height: 33px; background-color: #4B49AC; border-radius: 10px; color: white; padding: 5px 5px 5px 5px; "><p class="text-center text-truncate" style="max-width: 120px;height:19px" data-toggle="tooltip" title="' + text + '">' + text + '</p><i class="ml-1 ti-close remove_selected_category" style="color: white; cursor: pointer; font-size: 10px; "></i></div>';
