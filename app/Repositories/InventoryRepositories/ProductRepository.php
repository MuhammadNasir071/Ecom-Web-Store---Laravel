...
class ProductRepository
{
    ...
    public function store($request, $imagesPaths)
    {
        ...
        $product = Product::create([
            ...
            'details' => $request->product_description,
            ...
            'product_managed_by' => $request->product_managed_by,
    ...
    {
        return Product::where('id', $productId)->with(['media', 'categories.allParentCategories'])->first();
    }
    ...
    public function update($request, $id, $imagesPaths)
    {
        ...
        $product->product_managed_by = $request->product_managed_by;
        ...
        $prevImgsIds = array_map(function ($v) {
            return (int) trim($v, "'");
        }, explode(",", $request->prev_img_ids));
    ...
    public function getSingleProductDetail($productId)
    {
        return Product::where('id', $productId)->with('media', 'brand', 'favoriteProduct', 'categories.subCategories')->get();
    }

    // Store viewd product 
    public function storeViewProduct($productId)
    {
        if($viewedProduct = ViewProduct::where('user_id', auth()->id())->where('product_id', $productId)->first())
        {
            ViewProduct::where('user_id', auth()->id())->where('product_id', $productId)->update([
                'product_viewed_count' => $viewedProduct->product_viewed_count + 1,
                'updated_at' => now(),
            ]);
        }
        else
    ...
    {
        if(FavoriteProduct::where('user_id', auth()->id())->where('product_id', $request->product_id)->first())
        {
            ViewProduct::where('user_id', auth()->id())->where('product_id', $request->product_id)->update([
                'user_id' => auth()->id(),
    ...
    {
        return FavoriteProduct::where('user_id', auth()->id())->where('product_id', $request->product_id)->delete();
    }

    // all  favorite product 
    public function getAllFavoriteProduct()
    {
       return Auth::user()->favoriteProduct()->with('media', 'categories')->get();
    }

    // Fetch all Viewed products 
    public function getViewedProduct()
    {
        return Auth::user()->getViewedProduct()->with('categories')->take(12)->get();
    }

    // Fetch all Viewed products 
    public function getPopularProduct()
    {
        return Product::orderBy('sold_count', 'desc')->with('media', 'categories')->orderBy('updated_at')->take(12)->get();
    }
}
