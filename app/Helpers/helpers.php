...
if (!function_exists('geoIPInfo')) {
    function geoIPInfo()
    {
        return geoip()->getLocation($_SERVER['REMOTE_ADDR'])->toArray();
    }
}
if (!function_exists('getSessionTimezone')) {
    function getSessionTimezone()
    {
        try {
            $geopipInfo = geoIPInfo();
            $timezone = $geopipInfo['timezone'];
...
if (!function_exists('toLocalTimezone')) {
    function toLocalTimezone($timestamp = null)
    {
        $timezone = Auth::check() && !empty(auth()->user()->timezone) ? auth()->user()->timezone : getSessionTimezone();

        if (is_null($timestamp)) {
            return Carbon::now()->timezone($timezone);
        } elseif ($timestamp == 'today') {
            return Carbon::today()->timezone($timezone);
        }
...
if (!function_exists('toUserLocalTimezone')) {
    function toUserLocalTimezone($user = null, $timestamp = null)
    {
        $timezone = Auth::check() && !empty(auth()->user()->timezone) ? auth()->user()->timezone : getSessionTimezone();
...
if (!function_exists('toServerTimezone')) {
    function toServerTimezone($timestamp = null)
    {
        ...
        return Carbon::parse($timestamp, Auth::check() && !empty(auth()->user()->timezone) ? auth()->user()->timezone : getSessionTimezone())->timezone($timezone);
    }
}

if (!function_exists('sendPushNotificationAll')) {
    function sendPushNotificationAll($user_fcm_token, $notification)
    {
        $url = 'https://fcm.googleapis.com/fcm/send';

        $SERVER_API_KEY = config()->get('services.firebase.apiKey');

        // payload data, it will vary according to requirement
        $data = [
            "registration_ids" => $user_fcm_token, // for multiple device ids
...
if (!function_exists('sendPushNotification')) {
    function sendPushNotification($user_fcm_token, $notification, $notification_data = [])
    {
        $url = 'https://fcm.googleapis.com/fcm/send';

        $SERVER_API_KEY = config()->get('services.firebase.apiKey');

        if ($SERVER_API_KEY != '' && !is_null($SERVER_API_KEY)) {
            // payload data, it will vary according to requirement
            $data = [
                "registration_ids" => $user_fcm_token, // for multiple device ids
...
if (!function_exists('structuredNotification')) {
    function structuredNotification($notification)
    {
        ...
        $notification['node_id'] = ($notification['node_id']) ? "?node_id=" . $notification['node_id'] : "";
...
if (!function_exists('getDecryptedBearerToken')) {
    function getDecryptedBearerToken($token)
    {
        if (Auth::user()) {
...
if (!function_exists('get_picture')) {
    ...
    function get_picture($picture, $type)
    {
        if ($picture == "") {
            switch ($type) {
                case 'male':
                    $picture = url('/') . '/assets/images/blank-profiles/blank_profile_male.jpg';
                    break;

                case 'female':
                    $picture = url('/') . '/assets/images/blank-profiles/blank_profile_female.jpg';
                    break;

                case 'other':
                    $picture = url('/') . '/assets/images/blank-profiles/blank_profile_other.jpg';
                    break;
...
if (!function_exists('get_picture_without_url')) {
    function get_picture_without_url($type)
    {
        switch ($type) {
            case 'male':
                $picture = '/assets/images/blank-profiles/blank_profile_male.jpg';
                break;

            case 'female':
                $picture = '/assets/images/blank-profiles/blank_profile_female.jpg';
                break;

            case 'other':
                $picture = '/assets/images/blank-profiles/blank_profile_other.jpg';
                break;
